...
/***/ function(module, exports, __webpack_require__) {
	...
	var _CanvasWindow = __webpack_require__(2);

	var _CanvasWindow2 = _interopRequireDefault(_CanvasWindow);
	...
	function BBOX(_ref) {
	  ...
	  if (img == null || canvasContainer == null) {
	    console.warn('BBOX: Missing some arguments in here...', 'canvasContainer: ' + (typeof canvasContainer === 'undefined' ? 'undefined' : _typeof(canvasContainer)), 'img: ' + (typeof img === 'undefined' ? 'undefined' : _typeof(img)));
	    return undefined;
	  }
	  ...
	  if (_ismobilejs2.default.phone || _ismobilejs2.default.tablet) {
	    down = 'touchstart';
	    move = 'touchmove';
	    up = 'touchend';
	  }
	  ...
	  var limitWidth = canvasContainer.offsetWidth;
	  var limitHeight = canvasContainer.offsetHeight;
	  ...
	  while (imageWidth > limitWidth || imageHeight > limitHeight) {
	    ...
	    var res = resizeImage(image, limitWidth, limitHeight);
	  ...
	  var cw = (0, _CanvasWindow2.default)({
	    canvas: canvas
	  });
	  ...
	  // EVENT LISTENERS
	  var md = _rx2.default.Observable.fromEvent(canvas, down).subscribe(onMousedown);
	  styleCursorListener();
	  ...
	  function setBbox(bbox) {
	    if (bbox == null || (typeof bbox === 'undefined' ? 'undefined' : _typeof(bbox)) !== 'object') {
	      console.warn('wrong argument type: expected Object and got ', typeof bbox === 'undefined' ? 'undefined' : _typeof(bbox));
	      return undefined;
	    ...
	    for (var coord in bbox) {
	      if (bbox.hasOwnProperty(coord)) {
	        if (typeof bbox[coord] !== 'number') {
	          console.warn('wrong argument type: expected number and got ', _typeof(bbox[coord]));
	          return undefined;
	        }
	      }
	    }
	    ...
	    var width = (bbox.x2 - bbox.x1) * ratio;
	    var height = (bbox.y2 - bbox.y1) * ratio;
	  ...
	  function onMousedown(md) {
	    ...
	    if (callback) {
	      ...
	      _rx2.default.Observable.fromEvent(canvasContainer, move).map(callback).takeUntil(_rx2.default.Observable.fromEvent(canvasContainer, up)).subscribe(redrawCanvas);

	      // Callback subscription, if there is any, on mouse up
	      _rx2.default.Observable.fromEvent(canvasContainer, up).take(1) // take only first event of series
	  ...
	  function _onMouseup(e) {
	    ...
	    if (cw.hasBbox() && subscription) {
	      ...
	      var origin = cw.getUpperLeftCorner();
	      var final = cw.getBottomRightCorner();
	  ...
	  function _styleCursor(e) {
	    var pos = _getPosition(e, canvasContainer);
	    var ep = cw.getEditPoint(pos);
	  ...
	  function resizeImage(image, limitWidth, limitHeight) {
	    if (typeof image === 'undefined' || typeof limitWidth === 'undefined' || typeof limitHeight === 'undefined') {
	  ...
	  function resizeWidth(image, limitWidth) {
	    ...
	    var ratio = limitWidth / image.width;
	    ...
	    return {
	      newImage: newImage, ratio: ratio
	    };
	  }

	  function resizeHeight(image, limitHeight) {
	    var newImage = document.createElement('img');
	    var ratio = limitHeight / image.height;
	  ...
	  function _getPosition(e) {
	    ...
	    if ((_ismobilejs2.default.phone || _ismobilejs2.default.tablet) && !e.touches.length) return;
	  ...
	  function styleCursorListener() {
	    return _rx2.default.Observable.fromEvent(canvasContainer, move).takeUntil(_rx2.default.Observable.fromEvent(canvasContainer, down)).subscribe(_styleCursor);
	  }
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.default = CanvasWindow;

	var _ismobilejs = __webpack_require__(1);

	var _ismobilejs2 = _interopRequireDefault(_ismobilejs);

	var _EditPoint = __webpack_require__(3);

	var _EditPoint2 = _interopRequireDefault(_EditPoint);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	function CanvasWindow(options) {
	  ...
	  function _CanvasWindow(options) {
	    ...
	    var minSize = 60;
	    var delta = 15;
	    if (_ismobilejs2.default.phone) delta = 30;
	    ...
	    // init values
	    var p1 = origin = { x: w / 3, y: h / 3 }; // upper left corner of a bbox
	    var p2 = opposite = { x: 2 * w / 3, y: 2 * h / 3 }; // bottom right corner of a bbox
	    var width = p2.x - p1.x;
	    ...
	    // MAIN RETURN
	    return {
	      getBottomRightCorner: getBottomRightCorner,
	      getUpperLeftCorner: getUpperLeftCorner,
	      getEditPoint: getEditPoint,
	      saveBbox: saveBbox,
	      isInside: isInside,
	      hasBbox: hasBbox,
	      resize: resize,
	    ...
	    function hasBbox() {
	      ...
	      if (Math.abs(p1.x - prevBbox.x) >= delta || Math.abs(p1.y - prevBbox.y) >= delta || Math.abs(width - prevBbox.width) >= delta || Math.abs(height - prevBbox.height) >= delta) {
	        return Math.abs(width) > delta && Math.abs(height) > delta;
	      }
	    ...
	    function getEditPoint(point) {
	      var selectedEPs = editPoints.filter(function (ep) {
	        return point.x > ep.x - delta && point.y > ep.y - delta && point.x < ep.x + delta && point.y < ep.y + delta;
	      });
	    ...
	    function isInside(point) {
	      return point.x > Math.min(origin.x, opposite.x) && point.x < Math.max(origin.x, opposite.x) && point.y > Math.min(origin.y, opposite.y) && point.y < Math.max(origin.y, opposite.y);
	    }
	    ...
	    function _createEditPoint(origin, id, style) {
	      return (0, _EditPoint2.default)({ origin: origin, id: id, style: style }).draw(ctx);
	    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(6)(module), (function() { return this; }()), __webpack_require__(4)))
