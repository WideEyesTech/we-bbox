...
/***/ function(module, exports, __webpack_require__) {
	...
	var Bbox = function Bbox(options) {
	  ...
	  function _Bbox(_ref) {
	    ...
	    if (imageWidth > limitWidth) {
	      var res = _resizeImage(img, limitWidth);
	      image = res.newImage;
	    } else {
	      image = img;
	    }
	    ...
	    // EVENT LISTENERS
	    if (!_ismobilejs2.default.phone) {
	      var _md = _rx2.default.Observable.fromEvent(canvas, down).flatMap(_onMousedown).subscribe(_redrawCanvas);

	      var _mu = _rx2.default.Observable.fromEvent(canvasContainer, up).flatMap(_onMouseup).subscribe(_styleCursor);

	      _rx2.default.Observable.fromEvent(canvasContainer, move).takeUntil(_rx2.default.Observable.fromEvent(canvasContainer, down)).subscribe(_styleCursor);
	    } else {
	      var _md2 = _rx2.default.Observable.fromEvent(canvas, down).flatMap(_onMousedown).subscribe(_redrawCanvas);

	      var _mu2 = _rx2.default.Observable.fromEvent(canvasContainer, up).flatMap(_onMouseup);
	    }
	    ...
	    function _onMousedown(md) {
	      ...
	      return _rx2.default.Observable.fromEvent(canvasContainer, move).map(callback).takeUntil(_rx2.default.Observable.fromEvent(canvasContainer, up));
	    }
	    ...
	    function _onMouseup(e) {
	      ...
	      return _rx2.default.Observable.fromEvent(canvasContainer, move).takeUntil(_rx2.default.Observable.fromEvent(canvasContainer, down));
	    }
	    ...
	    function _handleRectDraw(e) {
	      var res = _getRectangle(curPos, e);
	      return {
	        type: 'draw',
	        data: res
	      };
	...
	function CanvasWindow(options) {
	  ...
	  function _CanvasWindow(options) {
	    ...
	    function hasBbox() {
	      ...
	      if (Math.abs(p1.x - currentBbox.x) >= delta || Math.abs(p1.y - currentBbox.y) >= delta || Math.abs(width - currentBbox.width) >= delta || Math.abs(height - currentBbox.height) >= delta) {
	        return Math.abs(width) > delta && Math.abs(height) > delta;
	      }
